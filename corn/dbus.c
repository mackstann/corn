...
#define BRUNT_BUS_SERVICE "org.mpris.brunt"

#define BRUNT_BUS_ROOT_PATH "/"
#define BRUNT_BUS_PLAYER_PATH "/Player"
#define BRUNT_BUS_TRACKLIST_PATH "/TrackList"

#define BRUNT_BUS_INTERFACE "org.freedesktop.MediaPlayer"
...
mpris_register_objects(DBusGConnection * bus)
{
    ...
    dbus_g_proxy_call(
        ...
        G_TYPE_STRING, BRUNT_BUS_SERVICE, G_TYPE_UINT, 0, G_TYPE_INVALID,
    ...
    MprisRoot * root = g_object_new(BRUNT_TYPE_MPRIS_ROOT, NULL);
    MprisPlayer * player = g_object_new(BRUNT_TYPE_MPRIS_PLAYER, NULL);
    MprisTrackList * tracklist = g_object_new(BRUNT_TYPE_MPRIS_TRACKLIST, NULL);

    // these object info thingies are generated by dbus-glib-tool from the xml
    // file and put into the glue header.
    dbus_g_object_type_install_info(BRUNT_TYPE_MPRIS_ROOT, &dbus_glib_mpris_root_object_info);
    dbus_g_object_type_install_info(BRUNT_TYPE_MPRIS_PLAYER, &dbus_glib_mpris_player_object_info);
    dbus_g_object_type_install_info(BRUNT_TYPE_MPRIS_TRACKLIST, &dbus_glib_mpris_tracklist_object_info);

    dbus_g_connection_register_g_object(bus, BRUNT_BUS_ROOT_PATH, G_OBJECT(root));
    dbus_g_connection_register_g_object(bus, BRUNT_BUS_PLAYER_PATH, G_OBJECT(player));
    dbus_g_connection_register_g_object(bus, BRUNT_BUS_TRACKLIST_PATH, G_OBJECT(tracklist));
